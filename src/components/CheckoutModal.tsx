...
export default function CheckoutModal({ isOpen, onClose, tier, onConfirm, creatorName, creatorId }: CheckoutModalProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-300 transform">
        {/* Header */}
        <div className="relative h-28 bg-gradient-to-br from-[#9810fa] to-[#e60076] flex items-center justify-center">
          {showPayment && (
            <button 
              onClick={() => setShowPayment(false)}
              className="absolute top-4 left-4 p-2 bg-white/20 hover:bg-white/30 rounded-full transition-colors text-white"
            >
          ...
          <div className="text-center">
             ...
             <h2 className="text-2xl font-bold text-white tracking-tight">
                {showPayment ? 'Payment Details' : 'Complete Subscription'}
             </h2>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
             <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center text-xl font-bold text-purple-600 shadow-inner">
               {creatorName[0]?.toUpperCase()}
             </div>
             <div>
               <h3 className="text-lg font-bold text-gray-900">{creatorName}</h3>
               <p className="text-purple-600 font-semibold text-sm">{tier.name} Tier • ${finalPrice.toFixed(2)}/mo</p>
             </div>
          </div>
              ...
              {isProcessing && (
                 <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-50">
                   <div className="flex flex-col items-center">
                     ...
                     <p className="font-bold text-purple-900">Activating Subscription...</p>
                   </div>
                 </div>
              )}
            </div>
